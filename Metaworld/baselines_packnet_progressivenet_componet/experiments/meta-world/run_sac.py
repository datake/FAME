...
# python run_experiments.py --algorithm packnet --seed 0 --start-mode 0 --task-sequence 6 --cuda-device 3
# python3 run_sac.py --model-type=packnet --task-id=0 --seed=0 --save-dir=agents

# python run_experiments.py --algorithm prognet --seed 0 --start-mode 0 --task-sequence 6 --cuda-device 3
# python run_experiments.py --algorithm simple --seed 0 --start-mode 0 --task-sequence 6 --cuda-device 3
# python run_experiments.py --algorithm componet --seed 0 --start-mode 0 --task-sequence 6 --cuda-device 3
...
class Actor(nn.Module):
    ...
    def get_action(self, x, **kwargs):
        ...
        # print('device mean:',mean.device, log_std.device)
        std = log_std.exp()
        normal = torch.distributions.Normal(mean, std)
        # print('device normal:',normal.loc.device, normal.scale.device)
        x_t = normal.rsample()  # for reparameterization trick (mean + std * N(0,1))
        y_t = torch.tanh(x_t)
        # print('device y_t:',y_t.device)
        # print('device action_bias:',self.action_bias.device, self.action_scale.device)
...
#
#             ep_ret += reward
#             print(step, reward, termination, truncation, info.keys())
#
...
def evaluate_agent(agent, task_id,task_sequence, num_evals,device):
    ...
    while len(episodic_returns) < num_evals:
        ...
        step+=1
        ep_ret += reward
        # print( step,reward, terminated, truncated, info.keys())
        if "episode" in info:
...
if __name__ == "__main__":
    ...
    print(f"\n*** Run name: {run_name} ***\n") # Run name: task_0__packnet__run_sac__0
    if args.track:
        import wandb
    ...
    # env setup
    envs = gym.vector.SyncVectorEnv([make_env(args.task_id,args.task_sequence)]) # only one env
    ...
    if args.model_type == "simple":
        ...
        if len(args.prev_units) == 0:
            ...
            model = PackNetAgent.load(
                ...
                restart_heads=True, # new head para for each task?
                freeze_bias=True,
                map_location=device,
            ).to(device)
    ...
    for global_step in range(args.total_timesteps):
        ...
        # print('initial action: global_step',global_step)
        ...
        #         # print(
        #         #     f"global_step={global_step}, episodic_return={info['episode']['r']}, success={info['success']}" # global_step=4499, episodic_return=[510.71048], success=0.0
        #         # )
        #         writer.add_scalar(
        ...
        # if global_step % 2500 == 0:
            ...
            intermediate_stats['time'].append(round((time.time() - start_time) / 3600, 3)) # in hours
            intermediate_stats['count_success'].append(-1) # no use here

    log_path = 'log/'
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    intermediate_stats = pd.DataFrame(intermediate_stats)
    # intermediate_stats.to_csv(log_path + "/" + str(args.task_id+1) + '_' + task_name + ".csv", index=False)
