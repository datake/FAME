...
class SquashedNormal(pyd.transformed_distribution.TransformedDistribution):
    def __init__(self, loc, scale):
        self.loc = loc
        self.scale = scale
        # print('loc:',loc[0], scale[0],loc.shape, scale.shape)
...
class DiagGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, hidden_dim, hidden_depth, log_std_bounds):
        ...
        # output_dim is 2 * action_dim, includes mu and log_std
        # self.trunk = utils.mlp_tanh(obs_dim, hidden_dim, 2 * action_dim, hidden_depth)

        self.outputs = dict()
        self.apply(utils.weight_init)

    def forward(self, obs):
        mu, log_std = self.trunk(obs).chunk(2, dim=-1) # mu and log_std are in one vector, chunk splits it in to mu and log_std
        # print('obs',obs,mu,log_std)
